...
namespace Transport
{
    public class TcpCommandHandler
    {
        ...
        public async Task HandleUserInput(Socket socket, Arguments args)
        {
            while (true)
            {
                var input = Console.ReadLine(); // vstup od uzivatela
                if (input == null) 
                {
                ...
                var tokens = input.Split(' ', StringSplitOptions.RemoveEmptyEntries); //rozdelenie prikazov a ziskavanie prveho pre auth
                ...
                // switch s kontrolami prikazov 
                using var stream = new NetworkStream(socket, ownsSocket: false); // zabalenie cosketu na writeasync
                switch (tokens[0])
                {
                    case "/help":
                        Udp.HandleHelp();
